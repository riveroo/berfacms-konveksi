...
>
    ...
    <div class="container mx-auto px-6 flex items-center justify-between">
        ...
        <nav class="hidden lg:flex items-center gap-8 text-sm font-semibold tracking-wide uppercase">
            <a href="{{ url('/') }}#beranda" class="transition-colors hover:text-indigo-500" :class="(scrolled || !window.location.pathname.match(/^\/?$/)) ? 'text-slate-600' : 'text-slate-200'">Beranda</a>
            <a href="{{ url('/') }}#layanan" class="transition-colors hover:text-indigo-500" :class="(scrolled || !window.location.pathname.match(/^\/?$/)) ? 'text-slate-600' : 'text-slate-200'">Layanan</a>
            <a href="{{ url('/') }}#portofolio" class="transition-colors hover:text-indigo-500" :class="(scrolled || !window.location.pathname.match(/^\/?$/)) ? 'text-slate-600' : 'text-slate-200'">Portofolio</a>
            <a href="{{ url('/') }}#tentang" class="transition-colors hover:text-indigo-500" :class="(scrolled || !window.location.pathname.match(/^\/?$/)) ? 'text-slate-600' : 'text-slate-200'">Tentang Kami</a>
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('cart.index') }}" class="relative p-2 transition-colors" :class="(scrolled || !window.location.pathname.match(/^\/?$/)) ? 'text-slate-600 hover:text-indigo-500' : 'text-white hover:text-indigo-300'">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
                </svg>
                @php $cartCount = count(session('cart', [])); @endphp
                <span x-show="{{ $cartCount }} > 0" class="absolute top-0 right-0 bg-rose-500 text-white text-[10px] font-bold px-1 rounded-full min-w-[16px] h-4 flex items-center justify-center">
                    {{ $cartCount }}
                </span>
            </a>

            <x-button variant="indigo" href="{{ route('public.stock') }}" class="hidden sm:inline-flex rounded-full px-6 shadow-xl shadow-indigo-600/20">
                CEK STOCK
            </x-button>
    ...
    >
        ...
        <div class="mt-auto">
            <x-button variant="indigo" href="{{ route('public.stock') }}" class="w-full py-4 rounded-2xl text-lg" @click="mobileMenu = false">
                CEK STOCK
            </x-button>
        </div>
    </div>
