<x-filament-panels::layout>
    <div class="space-y-6" x-data="{ 
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div>
                <x-text variant="title">Product Pricing</x-text>
                <x-text variant="muted" class="mt-1">Manage Cost of Goods Sold (COGS) and Selling Price for all size stocks.</x-text>
            </div>
        ...
        <form method="GET" action="{{ route('admin.product-pricing') }}"
            ...
            <div class="flex-1 min-w-[240px]">
                <x-text variant="label" class="mb-1.5 ml-1 text-[10px]">Search Product or Variant</x-text>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800 text-sm">
                    <thead class="bg-gray-50 dark:bg-gray-800/50 uppercase text-[10px] text-gray-500 font-bold tracking-wider sticky top-0 z-10 shadow-sm">
                        <tr>
                            <th scope="col" class="px-6 py-4 text-left whitespace-nowrap">Stock ID</th>
                            <th scope="col" class="px-6 py-4 text-left whitespace-nowrap">Product Name</th>
                            <th scope="col" class="px-6 py-4 text-left whitespace-nowrap">Variant Name</th>
                            <th scope="col" class="px-6 py-4 text-left whitespace-nowrap">Color</th>
                            <th scope="col" class="px-6 py-4 text-center whitespace-nowrap">Size</th>
                            <th scope="col" class="px-6 py-4 text-right whitespace-nowrap">COGS</th>
                            <th scope="col" class="px-6 py-4 text-right whitespace-nowrap">Selling Price</th>
                            <th scope="col" class="px-6 py-4 text-center whitespace-nowrap">Action</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-gray-400 dark:text-gray-500 bg-gray-50/50 dark:bg-gray-800/10">
                                    <div class="flex flex-col items-center justify-center space-y-2">
                                        ...
                                        <p class="font-semibold text-sm">No stock pricing records found.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div x-cloak x-show="isEditModalOpen" 
            ...
            <div @click.away="isEditModalOpen = false" 
                ...
                <div>
                    <x-text variant="title" class="text-xl font-bold">Edit Pricing</x-text>
                    <x-text variant="muted" class="text-xs mt-1">Manage purchase cost (COGS) and selling price for this size option.</x-text>
                </div>
                ...
                <form :action="'{{ route('admin.product-pricing.update', '__ID__') }}'.replace('__ID__', activeStockId)" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4 p-3 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800/50 text-xs">
                        <div>
                            <span class="text-gray-400 block uppercase font-bold tracking-wider">Product / Variant</span>
                            <span class="font-bold text-gray-700 dark:text-gray-200" x-text="activeProductName + ' - ' + activeVariantName"></span>
                        </div>
                        <div>
                            <span class="text-gray-400 block uppercase font-bold tracking-wider">Size</span>
                    ...
                    <!-- COGS Input -->
                    <div>
                        <x-text variant="label" class="mb-1.5">Cost of Goods Sold (COGS) / HPP <span class="text-red-500">*</span></x-text>
                        <div class="relative">
                    ...
                    <!-- Selling Price Input -->
                    <div>
                        <x-text variant="label" class="mb-1.5">Selling Price / Harga Jual <span class="text-red-500">*</span></x-text>
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-semibold">Rp</span>
        ...
        <div x-cloak x-show="isImportModalOpen" 
            ...
            <div @click.away="isImportModalOpen = false" 
                ...
                <div>
                    <x-text variant="title" class="text-xl font-bold">Import Bulk Pricing</x-text>
                    <x-text variant="muted" class="text-xs mt-1">Upload Excel spreadsheet to bulk update COGS and selling prices.</x-text>
                </div>
                ...
                <form action="{{ route('admin.product-pricing.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div class="bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-100 dark:border-indigo-900/50 p-4 rounded-xl text-xs space-y-2">
                        <span class="block font-bold text-indigo-700 dark:text-indigo-400 uppercase tracking-wide">Import Instructions:</span>
                        <ul class="list-disc pl-4 text-gray-600 dark:text-gray-400 space-y-1">
                            <li>Always download the current template first.</li>
                            <li>Do not modify the <b>stock_id</b> column.</li>
                            <li>If Product Name, Variant Name, or Size mismatch with database records, that row will be skipped safely.</li>
                        </ul>
                    </div>

                    <!-- File input -->
                    <div>
                        <x-text variant="label" class="mb-1.5">Upload Spreadsheet File</x-text>
