<x-filament-panels::layout>
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <x-text variant="title">{{ __('transaction.transaction_list') }}</x-text>
                <x-text variant="muted" class="mt-1">{{ __('transaction.manage_transactions') }}</x-text>
            </div>
            <div x-data="{ showDropdown: false }" class="relative">
                <x-button @click="showDropdown = !showDropdown" variant="primary">
                    ...
                    {{ __('transaction.add_transaction') }}
                </x-button>
                <div x-show="showDropdown" @click.away="showDropdown = false" x-cloak
                    ...
                    <a href="{{ route('transactions.create', ['type' => 'pre_order']) }}"
                        class="block px-4 py-3 hover:bg-gray-50 border-b border-gray-100 font-medium text-sm text-gray-700">{{ __('transaction.pre_order') }}</a>
                    <a href="{{ route('transactions.create', ['type' => 'direct_order']) }}"
                        class="block px-4 py-3 hover:bg-gray-50 font-medium text-sm text-gray-700">{{ __('transaction.direct_order') }}</a>
                </div>
            </div>
        </div>
        ...
        <form method="GET" action="{{ route('transactions.index') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                {{-- Row 1: Search --}}
                <div>
                    <x-text variant="label" class="mb-1.5">{{ __('transaction.search_name_phone') }}</x-text>
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="{{ __('transaction.client_placeholder') }}"
                ...
                <div>
                    <x-text variant="label" class="mb-1.5">{{ __('transaction.trx_id') }}</x-text>
                    <input type="text" name="trx_id" value="{{ request('trx_id') }}" placeholder="{{ __('transaction.trx_placeholder') }}"
                        class="w-full h-10 px-4 text-sm rounded-lg border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-gray-900/20 focus:border-gray-900 dark:focus:ring-white/20 dark:focus:border-white outline-none transition">
                </div>

                {{-- Row 1: Status Dropdown --}}
                <div>
                    <x-text variant="label" class="mb-1.5">{{ __('transaction.trx_status') }}</x-text>
                    <select name="status"
                ...
                {{-- Row 2: Date From --}}
                <div>
                    <x-text variant="label" class="mb-1.5">{{ __('transaction.date_from') }}</x-text>
                ...
                {{-- Row 2: Date To --}}
                <div>
                    <x-text variant="label" class="mb-1.5">{{ __('transaction.date_to') }}</x-text>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                                ...
                                <td class="px-4 py-4 text-center">
                                    ...
                                    <span
                                        ...
                                        {{ $trx->transaction_type === 'pre_order' ? __('transaction.pre_order') : __('transaction.direct_order') }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    @php
                                        ...
                                        $itemText = match ($trx->item_status) {
                                ...
                                <td class="px-4 py-4 text-center">
                                    @php
                                        ...
                                        $statusText = match ($trx->status) {
                                            'cancelled' => __('transaction.cancelled'),
                                            'on progress' => __('transaction.on_progress'),
                                            'done' => __('transaction.done'),
                                            default => '—',
                                        };
                                    @endphp
                                    <span
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="py-16 text-center text-gray-400 dark:text-gray-600">
                                    ...
                                    <p class="text-sm">{{ __('transaction.no_transactions') }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
