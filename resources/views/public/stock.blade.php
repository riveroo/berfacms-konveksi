    ...
    <main class="pt-32 pb-24">
        <div class="container mx-auto px-6">
            ...
            <div class="max-w-4xl mb-12">
                <span class="text-indigo-600 font-bold uppercase tracking-widest text-sm mb-4 block">Inventory
                    Check</span>
            ...
            <div class="grid grid-cols-1 gap-8">
                @forelse($products as $product)
                    <div
                        class="bg-white rounded-[2.5rem] overflow-hidden shadow-sm border border-slate-200 hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-500 group">
                        <div class="flex flex-col xl:flex-row">
                            <!-- Left: Product Info -->
                            <div
                                class="xl:w-1/3 p-8 bg-slate-50/50 border-b xl:border-b-0 xl:border-r border-slate-100 flex gap-6 items-center">
                                @if($product->thumbnail)
                                    <img src="{{ Storage::url($product->thumbnail) }}" alt="{{ $product->product_name }}"
                                        class="w-32 h-32 rounded-3xl object-cover shadow-md group-hover:scale-105 transition-transform duration-500 bg-white">
                                @else
                                    <div
                                        class="w-32 h-32 bg-slate-200 rounded-3xl flex items-center justify-center text-slate-400">
                                ...
                                <div>
                                    <div
                                        class="px-3 py-1 bg-indigo-100 text-indigo-600 rounded-full text-[10px] font-bold tracking-widest uppercase mb-3 inline-block">
                                        {{ $product->variants->first()->productType->name ?? 'Uncategorized' }}
                                    </div>
                                    <h3 class="font-outfit text-2xl font-black text-slate-900 mb-2">
                                        {{ $product->product_name }}</h3>
                                    <a href="{{ route('products.show', $product->id) }}"
                                        class="text-indigo-600 text-sm font-bold flex items-center gap-2 hover:text-indigo-700 transition-colors">
                                        LIHAT PRODUK
                                        ...
                                        @foreach($product->variants as $variant)
                                            <tr class="hover:bg-slate-50 transition-colors duration-300">
                                                <td class="flex items-center gap-3">
                                                    <div class="w-4 h-4 rounded-full border border-slate-200"
                                                        style="background-color: {{ strtolower($variant->color ?: '#ccc') }}">
                                                    </div>
                ...
                @empty
                    <div class="bg-white p-24 rounded-[3rem] text-center border border-slate-200">
                        <div
                            class="w-24 h-24 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-6 text-slate-300">
                            <svg class="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                            </svg>
                        ...
                        <x-button href="{{ route('public.stock') }}" variant="outline"
                            class="mt-8 rounded-2xl px-8 border-slate-200">LIHAT SEMUA</x-button>
                    </div>
                @endforelse
            </div>
            ...
            <div class="mt-24 p-12 bg-indigo-600 rounded-[3rem] text-center relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        <x-button href="{{ route('products.index') }}" variant="white"
                            class="px-12 rounded-2xl h-16 text-indigo-600 text-lg shadow-xl shadow-black/10">PESAN
                            SEKARANG</x-button>
