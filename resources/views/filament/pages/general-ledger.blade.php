<x-filament-panels::page>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-6 mb-2">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">{{ __('finance.choose_account') }}</label>
                <select 
                ...
                >
                    <option value="">{{ __('finance.choose_account') }}</option>
    ...
    @if(!$accountId)
        <div class="flex flex-col items-center justify-center p-12 text-center bg-white border border-gray-200 rounded-xl dark:bg-gray-900 dark:border-gray-800 shadow-sm">
            ...
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">{{ __('finance.no_account_selected') }}</h3>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 max-w-sm">{{ __('finance.select_account_instruction') }}</p>
        </div>
    @else
        <div class="space-y-4">
            <div class="flex justify-between items-center gap-4">
                <div>
                    <h3 class="text-base font-bold text-gray-900 dark:text-white">{{ __('finance.ledger_records') }}</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('finance.showing_posted_transactions') }}</p>
                </div>
