    ...
    <div class="px-8 py-8 mx-auto w-full max-w-4xl min-w-0">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-2xl font-bold tracking-tight text-gray-950 dark:text-white">Transaction Detail</h2>
            </div>
            
            <div class="flex gap-2">
                @if($cashBook->reference_type !== 'transfer')
                <a href="{{ route('cash-book.edit', $cashBook->id) }}" class="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">Edit</a>
                @endif
                <form action="{{ route('cash-book.destroy', $cashBook->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this transaction?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-4 py-2 bg-danger-600 text-white rounded-lg hover:bg-danger-500">Delete</button>
                </form>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-6">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4 border-b pb-2 dark:border-gray-800">Overview</h3>
                
                <div class="space-y-4">
                    <div>
                        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Date</span>
                        <span class="text-gray-900 dark:text-white font-medium">{{ $cashBook->date->format('d M Y') }}</span>
                    </div>
                    <div>
                        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Amount</span>
                        <span class="text-xl font-bold {{ $cashBook->type === 'in' ? 'text-success-600' : 'text-danger-600' }}">
                            {{ $cashBook->type === 'in' ? '+' : '-' }} Rp {{ number_format($cashBook->amount, 0, ',', '.') }}
                        </span>
                    </div>
                    <div>
                        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Type</span>
                        @if($cashBook->reference_type === 'transfer')
                            <span class="inline-block px-2 py-1 text-xs font-semibold bg-primary-100 text-primary-800 rounded-full dark:bg-primary-900/30 dark:text-primary-400">Transfer {{ ucfirst($cashBook->type) }}</span>
                        @else
                            <span class="inline-block px-2 py-1 text-xs font-semibold {{ $cashBook->type === 'in' ? 'bg-success-100 text-success-800 dark:bg-success-900/30 dark:text-success-400' : 'bg-danger-100 text-danger-800 dark:bg-danger-900/30 dark:text-danger-400' }} rounded-full">
                                Money {{ ucfirst($cashBook->type) }}
                            </span>
                        @endif
                    </div>
                    <div>
                        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Account (Cash/Bank)</span>
                        <span class="text-gray-900 dark:text-white">{{ $cashBook->account->name }}</span>
                    </div>
                    <div>
                        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Category / Counter Account</span>
                        <span class="text-gray-900 dark:text-white">{{ $cashBook->counterAccount->name ?? '-' }}</span>
                    </div>
                    @if($cashBook->client_id)
                    <div>
                        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Client</span>
                        <span class="text-gray-900 dark:text-white font-semibold">{{ $cashBook->client->client_name }}</span>
                        <span class="text-xs text-gray-500">({{ ucfirst($cashBook->client->type) }})</span>
                    </div>
                    @endif
                    @if($cashBook->receive_from)
                    <div>
                        <span class="block text-sm font-medium text-gray-500 dark:text-gray-400">Receive From / Pay To</span>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-6">
                ...
                @if($cashBook->journalEntry)
                    <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
                        <table class="w-full text-sm text-left divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-800">
                                <tr>
                                    <th class="px-4 py-3 font-semibold text-gray-900 dark:text-gray-200">Account</th>
                                    <th class="px-4 py-3 font-semibold text-right text-gray-900 dark:text-gray-200">Debit</th>
                                    <th class="px-4 py-3 font-semibold text-right text-gray-900 dark:text-gray-200">Credit</th>
                                </tr>
                            </thead>
                    ...
                    <div class="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 text-sm rounded-lg flex items-start gap-2">
                        ...
                        <p>This journal entry is automatically maintained by the system. It ensures that your books are always balanced.</p>
                    </div>
                @else
                    <div class="text-center py-8 text-gray-500">
                        No journal entry found for this transaction.
                    </div>
                @endif
            </div>
        </div>
