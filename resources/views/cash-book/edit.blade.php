<x-filament-panels::layout>
    ...
    <div class="px-8 py-8 mx-auto w-full max-w-2xl min-w-0" x-data="cashBookForm()">
        <div class="flex items-center gap-4 mb-6">
            ...
            <h2 class="text-2xl font-bold tracking-tight text-gray-950 dark:text-white">Edit Transaction</h2>
        </div>
        ...
        <form action="{{ route('cash-book.update', $cashBook->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" required rows="2" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-lg shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="What is this transaction for?">{{ old('description', $cashBook->description) }}</textarea>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select name="counter_account_id" id="counter_account_id" x-model="counterId" required class="searchable-select">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ $cashBook->counter_account_id == $category->id ? 'selected' : '' }}>{{ $category->name }} ({{ $category->type }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <select name="client_id" id="client_id" class="searchable-select">
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->id }}" {{ $cashBook->client_id == $client->id ? 'selected' : '' }}>{{ $client->client_name }} ({{ ucfirst($client->type) }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Receive From / Pay To</label>
                        <input type="text" name="receive_from" value="{{ old('receive_from', $cashBook->receive_from) }}" class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-lg shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="e.g. John Doe (Manual Input)">
                    </div>
                </div>
            </div>

            {{-- PREVIEW WIDGET --}}
            <div x-show="isReadyForPreview()" class="bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                <h4 class="text-sm font-semibold text-gray-900 dark:text-white uppercase tracking-wider mb-4 flex items-center">
                    <svg class="w-4 h-4 mr-2 text-warning-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                    Updated Preview
                </h4>
                
                <p class="text-gray-600 dark:text-gray-300 mb-3">You are modifying a 
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('cashBookForm', () => ({
                type: '{{ old('type', $cashBook->type) }}',
                date: '{{ old('date', $cashBook->date->format('Y-m-d')) }}',
                amount: '{{ old('amount', floatval($cashBook->amount)) }}',
                accountId: '{{ old('account_id', $cashBook->account_id) }}',
                counterId: '{{ old('counter_account_id', $cashBook->counter_account_id) }}',
                accountName: '{{ $cashBook->account->name ?? '' }}',
                counterName: '{{ $cashBook->counterAccount->name ?? '' }}',
